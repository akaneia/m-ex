...
typedef enum PreloadEntryState // 0x0 of PreloadEntry
{
...
{
    PRELOADFILEKIND_0,       // special alloc? these will always result in an assertion?? 800180dc
...
struct PreloadEntry // scene changes wait for asyncronous disc reads to finish if they are logged as a PreloadEntry!
{
    ...
    u8 flags;                      // 0x4, bitflags of some sort. costume files are 0x1, plX files are 0x2, effect files are 0x4, anim files are 0x8, stage files and allocs are 0x10, IfAll/EfMnData/EfCoData/ItCo are 0x40, LbRb is 0x80
...
PreloadEntry *Preload_CreateEntry(int file_kind, char *filename, int r5, int heap_kind, int file_size, int is_init_archive, int r9, int flags);         // creates a preload entry for any given file/ (file_kind is 0 for stuff like fod reflection, r5 = 4, r9 = 7) 80017740
